...
- [x] Add a runtime Zod schema matching every existing `AnalysisResult` field.
- [x] Add the OpenRouter `openrouter/free` adapter with strict JSON Schema output.
- [x] Validate fresh and cached analysis payloads before returning them.
- [x] Record OpenRouter and the selected response model in cache provenance.
- [x] Keep direct DeepSeek and Gemini adapters exported but unreachable from normal execution.

### Task 2: OpenRouter-only RAG embeddings

**Files:** Modify `src/shared/lib/rag/embeddings.ts` and `scripts/ingest.ts`.

- [x] Point runtime and ingestion embedding calls to `https://openrouter.ai/api/v1` with `OPENROUTER_API_KEY`.
- [x] Use `openai/text-embedding-3-small` and preserve 1536 dimensions.
- [x] Keep a dormant direct OpenAI embedding adapter for future fallback.
...
- [x] Default and validate only `openrouter` in the demo API/UI.
- [x] Require `OPENROUTER_API_KEY`; keep direct keys optional.
- [x] Document that free chat routing does not make the preserved OpenAI embedding model free.

### Task 4: Verification

- [x] Run `npm run check:env`, `npm run check:routes`, `npm run typecheck`, `npm run lint`, and `npm run build`.
- [x] Run live smoke validation.
- [ ] Run a structured analysis after the user sets `OPENROUTER_API_KEY`.
- [x] Search for normal execution paths that still select direct providers.
